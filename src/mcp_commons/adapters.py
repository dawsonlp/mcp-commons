...
def create_mcp_adapter(use_case_method: Callable, *, 
                       custom_success_handler: Callable[[Any], dict[str, Any]] | None = None,
                       custom_error_handler: Callable[[Exception], dict[str, Any]] | None = None) -> Callable:
    """
    ...
    async def mcp_adapted_method(**kwargs) -> dict[str, Any]:
        ...
        try:
            ...
            if isinstance(result, UseCaseResult):
                return _handle_use_case_result(result, method_name, custom_success_handler)
            elif isinstance(result, McpCompatible):
                return result.to_mcp_result()
            else:
                # Handle raw results (for compatibility with non-standard use cases)
                logger.warning(f"Use case method '{method_name}' returned non-UseCaseResult: {type(result)}")
...
def _handle_use_case_result(result: UseCaseResult, method_name: str, 
                           custom_success_handler: Callable | None) -> dict[str, Any]:
...
def _handle_raw_result(result: Any, custom_success_handler: Callable | None) -> dict[str, Any]:
...
def _handle_exception(error: Exception, method_name: str, kwargs: dict[str, Any], 
                     custom_error_handler: Callable | None) -> dict[str, Any]:
    """Handle exceptions consistently."""
    logger.error(f"Error in MCP adapter for '{method_name}': {str(error)}", exc_info=True)
    ...
    return {
        ...
        "details": {
            "method": method_name,
            "parameters": {k: str(v) for k, v in kwargs.items()}  # Convert to strings for JSON serialization
        }
    }
...
def create_query_adapter(use_case_method: Callable) -> Callable:
    ...
    return create_mcp_adapter(use_case_method, custom_success_handler=query_success_handler)
...
def create_command_adapter(use_case_method: Callable) -> Callable:
    ...
    return create_mcp_adapter(use_case_method, custom_success_handler=command_success_handler)
...
def create_validation_adapter(use_case_method: Callable) -> Callable:
    ...
    return create_mcp_adapter(use_case_method, custom_error_handler=validation_error_handler)
