...
class BaseUseCase:
    ...
    def _log_success(self, operation: str, **context) -> None:
        ...
        self._logger.debug(f"Use case {operation} completed successfully", extra=context)

    def _log_error(self, operation: str, error: Exception, **context) -> None:
        """Log error during operation."""
        self._logger.error(
            f"Use case {operation} failed: {str(error)}", 
            extra=context,
            exc_info=True
        )

    def _handle_exception(self, error: Exception, operation: str, **context) -> UseCaseResult[Any]:
...
class QueryUseCase(BaseUseCase):
    ...
    async def execute_query(self, operation: str, query_func, **context) -> UseCaseResult[Any]:
...
class CommandUseCase(BaseUseCase):
    ...
    async def execute_command(self, operation: str, command_func, **context) -> UseCaseResult[Any]:
...
class UseCaseFactory:
    ...
    def create_use_case(self, use_case_class: type[BaseUseCase], **additional_dependencies) -> BaseUseCase:
